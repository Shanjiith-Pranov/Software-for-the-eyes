...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    while cap.isOpened():
        ...
        if toggle == 0:
            ...
            try: cv2.imshow('left-hand-crop', image[max(0,topleft_left[1]):min(height,bottomright_left[1]),max(0,topleft_left[0]):min(width,bottomright_left[0])])
            except: "Error displaying left hand"
            try: cv2.imshow('right-hand-crop', image[max(0,topleft_right[1]):min(height,bottomright_right[1]),max(0,topleft_right[0]):min(width,bottomright_right[0])])
            except: "Error displaying right hand"
